...
//            printf("[Log] ** set pmc: rcx(%d) (wrmsr -p%d %#x %#lx)\n", i, cpu, \
//                    CORE_X86_MSR_PERF_CTL + (2*i), perf_ctl);
...
char* compute_result_str(char* buf, size_t buf_len, char* desc, int counter) {
    ...
    snprintf(buf, buf_len, "%s: %s%lld.%.2lld (%lld)\n",
			desc, (result<0?"-":""), ll_abs(result/100), ll_abs(result)%100, (long long)(agg-agg_base)/100);
    return buf;
}
