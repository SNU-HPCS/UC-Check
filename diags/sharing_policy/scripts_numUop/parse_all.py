...
def main_parser(log_fname):
    ...
    re_pat = "numuop_(?P<first_cbsz>\d+)_(?P<numuop>\d+)_pid_(?P<first_pid>\d+)_(?P<second_pid>\d+)"
    ...
    with open(log_fpath) as f:
        for l in f:
            ...
            res_dict['code_block_sz_1st'].append(res["first_cbsz"])
            res_dict['numuop'].append(res["numuop"])
