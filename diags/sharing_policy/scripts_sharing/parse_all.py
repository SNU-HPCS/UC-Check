...
def main_parser(log_fname):
    ...
    re_pat = "numChunks_(?P<first_cn>\d+)_(?P<second_cn>\d+)_pid_(?P<first_pid>\d+)_(?P<second_pid>\d+)"
    ...
    res_dict = {
            ...
            'prog_type':[],
            'arch_name':[],
            'code_chunk_number_1st':[],
            'code_chunk_number_2nd':[],
            }
    ...
    with open(log_fpath) as f:
        for l in f:
            ...
            res_dict['code_chunk_number_1st'].append(res["first_cn"])
            res_dict['code_chunk_number_2nd'].append(res["second_cn"])
